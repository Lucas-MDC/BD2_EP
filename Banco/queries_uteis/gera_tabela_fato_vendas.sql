...
(
	...
	inner join (select count(id_cliente) num from cliente) cli
		on 1=1
	inner join rand_venda on rand_venda.i < 5 --Quantidade de linhas a serem geradas
)
select
...
(
	...
	inner join (select count(id_cliente) num from cliente) cli
		on 1=1
	inner join rand_venda on rand_venda.i < 200 --Quantidade de linhas a serem geradas
)
